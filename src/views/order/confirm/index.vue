...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      queryAddressList() {
        this.ajax({
          url: `/order/shipping/queryUserShippingListWithPage`,
          data: {
            current: 1,
            size: 50
          },
      ...
      queryOrderItemVoList() {
        this.ajax({
          url: `/order/order/getOrderCartProduct`,
          success: (res) => {
            if (!res || res.code !== '200') {
              this.loadPage('user-order');
            } else {
              this.productTotalPrice = res.data.productTotalPrice;
              this.orderItemVoList = res.data.orderItemVoList;
            }
          }
        });
      },
      createOrderDoc() {
        ...
        this.ajax({
          url: `/order/order/createOrderDoc/` + this.addressId,
          success: (res) => {
